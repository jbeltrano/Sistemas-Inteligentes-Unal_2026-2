...
class PicasFijasAgentCJBVC:
    ...
    # ------------------------------------------------------------------
    # PUNTO DE ENTRADA UNICO (compute): recibe [picas, fijas] y devuelve
    # la accion (el intento). En el primer turno no hay retroalimentacion,
    # asi que se llama compute() sin argumento. Es lo unico que llama el
    # ambiente; start/try_attempt/feedBack/discover se conservan intactos.
    # ------------------------------------------------------------------
    def compute(self, retroalimentacion=None):

        if retroalimentacion is not None:

            self.feedBack(retroalimentacion)
